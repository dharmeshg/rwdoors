...
Plugin URI: http://plugingarden.com/google-translate-wordpress-plugin/
...
function wp_translate_notice() {
	...
	$upgradeObject = $wpTranlsateOptions['4-9-upgrade_notice'];
	$upgradeObject['count']++;
	$upgradeObject['date'] = date('Y-m-d', strtotime('+3 Month'));
	
	$wpTranlsateOptions['4-9-upgrade_notice'] = $upgradeObject;	
...
function wp_translate_settings_javascript() { 
	...
	jQuery(document).ready(function($) {
		...
		jQuery('#btn-wp-transalate-settings').on('click', function() {
			...
			jQuery.post(ajaxurl, data, function(response) {
				jQuery('#wp-translate-update-status').empty().css('top', '-150px').append(response).show();
				$( "#wp-translate-update-status" ).animate({			
					top: "+=300"
				}, 1000, function() {
					// Animation complete.
					$( "#wp-translate-update-status" ).fadeOut(1500);
				});				
			});
...
function wpt_upgrade_notice() {
	...
	if ($showNotice) {
	?>
	<div id="wp-translate-notice" class="updated notice" style="clear: both;">
		<p style="float: left;">Want to give your readers a better translation experience? Check out <a href="http://plugingarden.com/google-translate-wordpress-plugin/?src=wpt" target="_blank">WP Tranlate Pro</a>.<br/><i>Show country flag icons next to languages and remove Google branding. Also has short code for pages that don't display widgets.</i></p>
